...
const HttpStatus = {
    ...
    BAD_REQUEST: { code: 400, status: 'NOT_FOUND' },
...
export const getUsers = (req, res) => {
    ...
    database.query(QUERY.SELECT_USERS, (errors, results) => {
        if (!results) {
            res.status(HttpStatus.OK.code)
                .send(new Response(HttpStatus.OK.code, HttpStatus.OK.status, `No users found`));
        } else {
            res.status(HttpStatus.OK.code)
                .send(new Response(HttpStatus.OK.code, HttpStatus.OK.status, `Users retrieved`, { patient: results }));
        }
    });
};
